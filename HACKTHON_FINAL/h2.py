...
def append_text_to_doc(text):
    ...
    run.font.bold = True
    run.font.size = Pt(12)  # Set font size to 48 points (96 half-points), similar to H1
    doc.save(name)
    stack.set_action(text)
    print(f"Appended text: {text}")
    

def add_h2_to_doc(parent):
    global root
    parent.destroy()
    root = None
...
# def create_h2(parent):
#     # Create a button with the text "Push"
#     button_push = tk.Button(parent, text="H2", bg="black", fg="white", font=("Arial", 7), padx=10, pady=5, command=lambda: add_text_to_doc(parent))
